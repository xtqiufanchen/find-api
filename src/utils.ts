...
export const getAst = (filePath: string) => {
  ...
  if (filePath.includes('.vue')) {
    ...
    content = sfcParseResult.descriptor.script?.content as string || sfcParseResult.descriptor.scriptSetup?.content as string;
    if (!content) {
...
export const resolveModulePath = (srcDir: string, currentFile: string, importPath: string): string | false | null => {
  ...
  if (fs.existsSync(resolvedPath) && fs.statSync(resolvedPath).isFile()) {
    if ([".png", ".less", ".svg"].includes(path.extname(resolvedPath))) {
      return false
    }
    return resolvedPath
  }
