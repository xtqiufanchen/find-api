...
const traverseImpl = (filePath: any, apiCalls: Record<string, any>, srcDir: string, result: Record<string, any>, onFindImport: (path: string, parentResult: Record<string, any>) => void) => {
...
export const findApiReferences = (filePath: string, apiCalls: Record<string, any>, srcDir: string): Record<string, any> => {
  ...
  while(eachQueue.length) {
    ...
    traverseImpl(filePath, apiCalls, srcDir, parentResult, (resolvedPath, result) => {
      ...
      if (referFiles[resolvedPath]) {
        ...
        Object.assign(
          result, referFiles[resolvedPath]
        )
        return null;
      }
