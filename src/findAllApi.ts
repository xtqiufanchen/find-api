...
const parseFile = (filePath: string) => {
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      if ((t.isMemberExpression(callee.node) && t.isIdentifier(callee.node.property, { name: 'get' }) && t.isIdentifier(callee.node.object, { name: 'http' })) || t.isIdentifier(callee.node, { name: 'request' })) {
        ...
        if (detail) {
          ...
          const exportDeclaration = functionParent.findParent(path => {
            return path.isDeclareExportDeclaration() || path.isExportDefaultDeclaration() || path.isExportNamedDeclaration();
          })
          ...
          const declaration = (exportDeclaration.node as t.ExportNamedDeclaration).declaration;
          if (!declaration) {
      ...
      } else if (t.isMemberExpression(callee.node) && t.isIdentifier(callee.node.property, { name: 'post' }) && t.isIdentifier(callee.node.object, { name: 'http' })) {
